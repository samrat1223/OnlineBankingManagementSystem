...
public class CustomerService implements CustomerCrud {
	...
	public int insertRecord(Customer customer) {
        ...
        try{
            ...
            PreparedStatement ps =con.prepareStatement("insert into customer(Cust_ID,Cust_Name,Cust_Aadharno,Cust_Address,Cust_Panno,Cust_Email,Cust_Password,Cust_MobileNo,Cust_PinCode,Cust_State,User_Name,Cust_DOB,Accnt_No,Cust_NomineeName) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?);");
	...
	public int updateRecord(Customer customer) {
        ...
        try{
            ...
            PreparedStatement ps =con.prepareStatement("update customer set Cust_Name=?,Cust_Aadharno=?,Cust_Address=?,Cust_Panno=?,Cust_Email=?,Cust_Password=?,Cust_MobileNo=?,Cust_PinCode=?,Cust_State=?,User_Name=?,Cust_DOB=?,Accnt_No=?,Cust_NomineeName=? where Cust_ID=?");
	...
	public List<Customer> getAllRecords() {
		...
		while(rs.next()) {
			...
			list.add(cust);
			//list.add(new Customer(rs.getInt(1),rs.getString(2),rs.getString(3),rs.getString(4),rs.getString(5),rs.getString(6),rs.getString(8),rs.getInt(7), rs.getInt(9),rs.getString(10),rs.getString(11),rs.getString(12),rs.getDate(13),rs.getInt(14)));
	...
	public Customer getCustomerByCust_ID(int Cust_ID) {
		...
		try{
			...
			if(rs.next()) {
				//customer=new Customer(rs.getInt(1),rs.getString(2),rs.getString(3),rs.getString(4),rs.getString(5),rs.getString(6),rs.getString(8),rs.getInt(7), rs.getInt(9),rs.getString(10),rs.getString(11),rs.getString(12),rs.getString(13),rs.getInt(14));
				cust  = new Customer();
